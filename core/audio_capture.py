...
class AudioCapture:
    ...
    def get_default_loopback(self) -> Optional[AudioDevice]:
        """Get the best WASAPI loopback device.

        Tries to find a non-virtual physical device first (headphones/speakers),
        ...
        # First: try physical output devices (headphones/speakers)
        for dev in devices:
        ...
        # Second: try non-virtual (including digital output)
        for dev in devices:
            name_lower = dev.name.lower()
            is_virtual = any(kw in name_lower for kw in virtual_keywords)
        ...
        # Fallback: use WASAPI default output's loopback
        pa = pyaudio.PyAudio()
        try:
    ...
    def start(self, device: Optional[AudioDevice] = None) -> None:
        with self._lock:
            ...
            src_rate = device.sample_rate
            src_ch = device.channels
            ...
            def audio_callback(in_data, frame_count, time_info, status):
                try:
                    ...
                    _LOGGER.error("Audio callback error: %s", e)
    ...
    @staticmethod
    def _convert_audio(data: bytes, src_rate: int, src_channels: int,
        ...
        if src_rate != dst_rate:
            ...
            if dst_frames > 0:
                ...
                resampled = np.zeros((dst_frames, samples.shape[1]), dtype=np.int16)
